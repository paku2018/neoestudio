...
<div class="content-wrapper">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"><a href="{{url('questions/edit/'.$question->id)}}">@lang('lng.Edit questions')</a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      <div class="row">
        <div class="col-12">
          <form method="POST" action="{{url('questions/update/'.$question->id)}}" enctype="multipart/form-data">
            {{ csrf_field() }}
                <div class="card-body container" style="width: 50%">
                  
                  
                  <input type="hidden" name="examId" value="{{$question->examId}}">
                  
                  <div class="form-group">
                    <label for="name">@lang('lng.Question')</label>
                    <textarea id="question" class="form-control" name="question"  placeholder="@if(!empty($question)){{$question->question}}@endif" value="@if(!empty($question)){{$question->question}}@endif">@if(!empty($question)){{$question->question}}@endif</textarea>
                  </div>
                  <div class="form-group">
                    <label for="name">@lang('lng.Answer1')</label>
                    <textarea id="answer1" class="form-control" name="answer1"  placeholder="@if(!empty($question)){{$question->answer1}}@endif" value="@if(!empty($question)){{$question->answer1}}@endif">@if(!empty($question)){{$question->answer1}}@endif</textarea>
                  </div>
                  <div class="form-group">
                    <label for="name">@lang('lng.Answer2')</label>
                    <textarea id="answer2" class="form-control" name="answer2"  placeholder="@if(!empty($question)){{$question->answer2}}@endif" value="@if(!empty($question)){{$question->answer2}}@endif">@if(!empty($question)){{$question->answer2}}@endif</textarea>
                  </div>
                  <div class="form-group">
                    <label for="name">@lang('lng.Answer3')</label>
                    <textarea id="answer3" class="form-control" name="answer3"  placeholder="@if(!empty($question)){{$question->answer3}}@endif" value="@if(!empty($question)){{$question->answer3}}@endif">@if(!empty($question)){{$question->answer3}}@endif</textarea>
                  </div>
                  <div class="form-group">
                    <label for="name">@lang('lng.Answer4')</label>
                    <textarea id="answer4" class="form-control" name="answer4"  placeholder="@if(!empty($question)){{$question->answer4}}@endif" value="@if(!empty($question)){{$question->answer4}}@endif">@if(!empty($question)){{$question->answer4}}@endif</textarea>
                  </div>
                  <div class="form-group">
                    <label for="name">@lang('lng.Correct')</label>
                    <input type="text" class="form-control" name="correct"  placeholder="@if(!empty($question)){{$question->correct}}@endif" value="@if(!empty($question)){{$question->correct}}@endif">
                  </div>
                  <div class="form-group">
                    <label for="name">@lang('lng.Description')</label>
                    <textarea id="description" class="form-control" name="description"  placeholder="@if(!empty($question)){{$question->description}}@endif" value="@if(!empty($question)){{$question->description}}@endif">@if(!empty($question)){{$question->description}}@endif</textarea>
                  </div>
                  <div class="form-inline">
                    <input type="checkbox" class="form-control" name="check"><label>@lang('lng.Use This') &nbsp; &nbsp;</label>
                    <img src="{{asset($question->image)}}" style="width: 50px; height: auto;">
                  </div>
